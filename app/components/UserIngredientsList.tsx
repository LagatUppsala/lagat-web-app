...
export default function UserIngredientList({ uid }: { uid: string }) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetch(
        "https://europe-north1-lagat-e1c30.cloudfunctions.net/add_ingredient",
        {
          method: "POST",
          headers: {
  ...
  const handleRemove = async (item: Ingredient) => {
    ...
    try {
      const token = await auth.currentUser!.getIdToken();
      await fetch(
        "https://europe-north1-lagat-e1c30.cloudfunctions.net/remove_ingredient",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        {showFridgeDesc && (
          <div className="absolute top-full left-0 mt-2 w-60 bg-white rounded-md shadow-lg p-4 z-20">
            <p className="text-md text-gray-600">
              Lägg till ingredienser i ditt kylskåp! Vi matchar ingredienserna
              mot recept så att du kan spara pengar och få matinspiration.
              Klicka på en ingrediens för att ta bort den.
            </p>
          </div>
        )}
      </div>
      <span className="text-gray-500 text-sm leading-thight">
        Du kan behöva vänta ett ögonblick efter du har lagt till ingredienser
        innan du kan se rätt rekommenderade recept i receptflödet!
      </span>
      <form
        onSubmit={handleAdd}
      ...
      <ul className="list-none space-y-2">
        {ingredients.length > 0 ? (
          ingredients.map((item) => (
            ...
            ${
              item.isOptimistic
                ? "bg-gray-200 animate-pulse text-gray-400"
